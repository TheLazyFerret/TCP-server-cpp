 ...
 * 
 * @brief common functions used by server and client library.
 *   Should not be needed to be included.
 */
...
in_addr tcp_shared::ConvertAddrBinary(const std::string& address) {
  in_addr addr;
...
std::string tcp_shared::ConvertAddrSring(const in_addr& address) {
...
int tcp_shared::InitializeSocket() {
...
/// @param socket_fd 
void tcp_shared::KillSocketfd(int& socket_fd) {
  if (socket_fd < 0) {
    return;
  }
...
size_t tcp_shared::Send(const void* src, const size_t len, const int flags, const int socket_fd) {
  if (src == nullptr) {
...
size_t tcp_shared::Recv(void* dst, const size_t len, const int flags, const int socket_fd) {
  if (dst == nullptr) {
    throw tcp_exception::InvalidPointer();
  }
